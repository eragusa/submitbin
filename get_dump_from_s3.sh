...
#
# Description:
# Stage a file in from S3 using the 's3' (s3lib) command.
...
usage_string="Usage: $0 [-b S3 bucket] inkey1 [,inkey2,...]"
...
for in_key in "$@"; do
  $my_s3 get "$fst_s3_bucket/$in_key" filename="$in_key" 
  if [ $? -ne 0 ]; then
    >&2 echo "$0: unable to s3 get $fst_bucket/$in_key."
